...
from src.data_augmentation import augment_sets_timesets
...
parser.add_argument('--splitsize', nargs='?', help='size of the sets, that the dataset is splitted into')
...
if args.__getattribute__('splitsize') == None:
    SPLITSIZE = 50
else:
    SPLITSIZE = int(args.__getattribute__('splitsize'))
...
RANDOM_SHUFFLE_PKL_PATH = 'data/random_shuffle'
RANDOM_SHUFFLE_TRAIN_PATH = 'data/random_shuffle/train'
RESULTS_RANDOM_SHUFFLE = 'results/result_random_shuffle.txt'
...
def main():
    ...
    def augment_dataset(path, times_to_aug):
        ...
        # shuffle timesets
        load_p_mw_da = augment_sets_timesets(load_p_mw_train, 'load_p_mw', times_to_aug=times_to_aug, \
                                             split_size=SPLITSIZE)
        load_p_mw_da = load_p_mw_da.reset_index(drop=True)
        load_p_mw_da.to_pickle('./'+path+'/load_p_mw_da.pkl')
        load_q_mvar_da = augment_sets_timesets(load_q_mvar_train, 'load_q_mvar', times_to_aug=times_to_aug, \
                                               split_size=SPLITSIZE)
        load_q_mvar_da = load_q_mvar_da.reset_index(drop=True)
        load_q_mvar_da.to_pickle('./'+path+'/load_q_mvar_da.pkl')
        sgen_p_mw_da = augment_sets_timesets(sgen_p_mw_train, 'sgen_p_mw', times_to_aug=times_to_aug, \
                                             split_size=SPLITSIZE)
    ...
    # da with random shuffle
    augment_dataset(path=RANDOM_SHUFFLE_PKL_PATH, times_to_aug=TIMES_TO_AUG)
    generate_y_data(train_path=RANDOM_SHUFFLE_TRAIN_PATH, pkl_path=RANDOM_SHUFFLE_PKL_PATH)
    run_with_da(x_path=RANDOM_SHUFFLE_PKL_PATH, \
                y_train_path=RANDOM_SHUFFLE_TRAIN_PATH, \
                y_test_path=WITHOUT_DA_TRAIN_PATH, times_to_aug=TIMES_TO_AUG, \
                additional_info='shuffle timesets sets of 100', res_file=RESULTS_RANDOM_SHUFFLE)
